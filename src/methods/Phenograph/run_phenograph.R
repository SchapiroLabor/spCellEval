...
#### Args ####
parser <- ArgumentParser(description = "Cluster data with PhenoGraph")
...
parser$add_argument("-k", "--k_phenograph", dest = "k_phenograph", nargs = "+",
                    type = "integer", default = c(20, 30, 40, 80),
                    help = "k values for PhenoGraph (default: 20 30 40 80)")
...
if (args$log != "off") {
  ...
  addHandler(writeToFile,
    file = paste0(args$output_path, "/logs/", Sys.Date(), "_phenograph.log"),
    level = "INFO",
...
for (iteration in 1:args$iterations) {
  for (k_val in args$k_phenograph) {
    ...
    if (args$log != "off")
      loginfo("Rphenograph k = %d (Duration: %.2f min)", k_val,
