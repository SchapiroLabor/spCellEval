...
class ClassicMLDefault:
    ...
    def _zscore_scaling(self, df, features, scale_batch_col=None):
            df_scaled = df.copy()
            df_scaled[features] = df_scaled[features].astype(np.float64)
            if scale_batch_col is not None and scale_batch_col in df.columns:
                for batch in df[scale_batch_col].unique():
                    batch_mask = df[scale_batch_col] == batch
                    df_scaled.loc[batch_mask, features] = self.scaler.fit_transform(df.loc[batch_mask, features])
            else:
                df_scaled[features] = self.scaler.fit_transform(df[features])
            return df_scaled


    def train_tune_evaluate(self, path: str, label_path:str, verbose:int = 2, scaling:bool = True, scale_batch_col:str = None, dumb_columns: str | list = None, dumb_nonnumericals: bool = True) -> None:
        """
        ...
        for i, (train_file, validation_file, test_file) in enumerate(zip(fold_dict['train'], fold_dict['validation'], fold_dict['test'])):
            ...
            if dumb_nonnumericals:
                def keep_batch_and_numerics(df):
                    if scale_batch_col and scale_batch_col in df.columns:
                        batch_col = df[[scale_batch_col]]
                        numeric_cols = df.select_dtypes(include=[np.number]).columns
                        return df[numeric_cols].join(batch_col)
                    return df.select_dtypes(include=[np.number])

                train_data = keep_batch_and_numerics(train_data)
                validation_data = keep_batch_and_numerics(validation_data)
                test_data = keep_batch_and_numerics(test_data)
            if scale_batch_col is not None and scale_batch_col in train_data.columns:
                non_numeric_df = train_data.drop(columns=scale_batch_col).select_dtypes(exclude=[np.number])
            else:
                non_numeric_df = train_data.select_dtypes(exclude=[np.number])
            if train_data.isnull().values.any() or validation_data.isnull().values.any() or test_data.isnull().values.any():
            ...
            if scaling:
                if scale_batch_col is not None and scale_batch_col in train_data.columns:
                    print(f"Scaling data with batch column {scale_batch_col}")
                    features = [col for col in train_data.columns if col not in ['encoded_phenotype', scale_batch_col]]
                    train_data = self._zscore_scaling(train_data, features, scale_batch_col)
                    validation_data = self._zscore_scaling(validation_data, features, scale_batch_col)
                    test_data = self._zscore_scaling(test_data, features, scale_batch_col)
                    train_data = train_data.drop(columns=scale_batch_col)
                    validation_data = validation_data.drop(columns=scale_batch_col)
                    test_data = test_data.drop(columns=scale_batch_col)
                
                else:
                    print(f"Scaling data without batch column")
                    features = [col for col in train_data.columns if col != 'encoded_phenotype']
                    train_data = self._zscore_scaling(train_data, features)
                    validation_data =self. _zscore_scaling(validation_data, features)
                    test_data = self._zscore_scaling(test_data, features)
