...
def run_kfold_creation(main_dir, dropna, impute_value, phenotype_column, drop_columns, drop_non_numerical, n_splits, random_state, percentage_validation):
    """
    ...
    if drop_non_numerical == 'Yes':
        drop_non_numerical = True
    else:
        drop_non_numerical = False
    ...
    for dataset in os.path.join(main_dir, 'datasets'):
        ...
        save_dir = os.path.join(dataset_path, 'quantification/processed')
...
def main():
    ...
    parser.add_argument(
        '--main_dir',
        type = str,
        help = "'Path to the main directory holding the folders 'datasets' and 'results'. An explicit directory structure is required. See README for more information",
        required = True,
    )
    parser.add_argument(
        '--dropna',
        type=str,
        help='drop rows with missing values. Default is No',
        default='No',
    )
    parser.add_argument(
        '--impute_value',
    ...
    parser.add_argument(
        '--drop_non_numerical',
        type = str,
        help = 'drop non-numerical columns. Default is No',
        default = 'No',
    )
    parser.add_argument(
        '--n_splits',
    ...
    run_kfold_creation(args.main_dir, args.dropna, args.impute_value, args.phenotype_column,
