from models_classes.default_classic_ml_models_kfolds_batchscale import ClassicMLDefault
import argparse
import os
import json

def run_on_datasets(main_dir, model, kfold_method, granularity_level, random_state, n_jobs_model, model_kwargs, verbose, scaling, scale_batch_col, dumb_columns):
    """
    ...
    for dataset_dir in os.listdir(os.path.join(main_dir, 'datasets')):
        ...
        data_object.train_tune_evaluate(kfold_dir, label_dir, verbose, scaling, scale_batch_col, dumb_columns)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Column to perform batch-wise scaling. If not provided, scaling will be performed on the entire dataset."
        )
    parser.add_argument(
        "--dumb_columns",
        type=str,
    ...
    run_on_datasets(
        ...
        scale_batch_col=args.scale_batch_col,
