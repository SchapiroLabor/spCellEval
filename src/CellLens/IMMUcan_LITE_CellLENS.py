...
parser = argparse.ArgumentParser(description='Running CellLENS lite on immucan.')
...
parser.add_argument('-l', '--log', dest='log', required= False, default='off', choices=['short', 'long', 'off'], help='Logging level: short, long, or off (default: off)')
...
def lite_celllens(input, markers, iterations, output_path, log, path2crops):    
    ...
    df["image_shift"] = df["image"].astype("category").cat.codes
    df["image_shift"] = df["image_shift"].astype("float")

    # add shift value to x coordinates to 'stitch' images together
    df['x'] = df['image_shift'] * 1000 + df['x']

    # mirror coordinates to match image coordinates in full celllens analysis
    df['x_new'] = df['y']
    df['y_new'] = df['x']
    ...
    for iteration in range(1, iterations + 1): 
        ...
        murine_dataset = LENS_Dataset(df,
                                    ...
                                    feature_neighbor=15,
                                    spatial_neighbor=15,
                                    path2img=f'{path2crops}/', # Not used - location to store cropped images
                                    use_transform = False) # Not used
        ...
        murine_dataset.initialize(cent_x="x_new", # x loc of cells
                                ...
                                cluster_res= 0.1 # leiden initialization resolution
...
#example usage: python3 IMMUcan_LITE_CellLENS.py -i ../../Maps_data/cHL_2_MIBI/quantification/processed/cHL_2_MIBI_quantification.csv -pc ../chl2/processed_images -o ../cHL2/ -m CD163 CD11c CD28 -l long -it 1
#immucan markers:
